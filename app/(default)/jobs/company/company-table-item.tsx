...
export default function CompaniesTableItem({
  company,
  onCheckboxChange,
  isSelected,
}: CompaniesTableItemProps) {
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    onCheckboxChange(company.id, e.target.checked);
  };
    ...
    <tr>
      ...
      <td className='px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap'>
        <Link href={`/jobs/company/${company.id}`}>
          <div className='font-medium text-sky-600'>{company.name}</div>
        </Link>
      </td>
      <td className='px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap'>
